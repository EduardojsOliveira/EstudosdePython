#Se eu colocar um nome ou escrever algo dentro das aspas simples(') ou duplas(") ele já identifica com str(string)
#Para escrever algum texto smepre ultilize aspas simples ou duplas
#Se colocar \" ou \' ele ignora a aspas e le oque esta escrito
#Se quiser que a \ aparecera tambem junto com a aspas ou só ela é ultilizado o comando r
#Normalmente nao vamos ultilizar muito o \' ou o r pois fica um pouco complicado
#É ultilizado aspas dentro de aspas quando vc querer mostrar os aspas no seu teminal
print(1234)
print('Eduardo José')
print('Eduardo \'José')
print(r'Eduardo \'José')
print(2,"Eduardo 'josé'", sep= '-')#O forma que é melhor e mais ultilizada para marcar o nome ou algo com aspas

